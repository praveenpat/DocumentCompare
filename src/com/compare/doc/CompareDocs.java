...
public class CompareDocs {
	...
	public static void main(String[] args) throws Exception {
		...
		//Document doc = Jsoup.connect("https://mbsdisclosure.fanniemae.com/PoolTalk2/securityDetails.html?cusip=31362TDF7").get();
		
		Document doc = Jsoup.connect("https://mbsdisclosure.fanniemae.com/PoolTalk2/securityDetails.html?cusip="+cusip).get();
		...
		List<String> recordsFromCsv = getRecordsFromCsv(csvUrl);
		...
		csvKeyVals.forEach((key,val) -> {
			...
			if(formattedKeyVals.get(key)!=null){
				...
				if(formattedKeyVals.get(key).equalsIgnoreCase(val)){
					...
					System.out.println("Key="+key +",csvValue="+ val + ",htmlValue="+formattedKeyVals.get(key)+ ",status=Match ");
					
				}
				else{
					
					System.out.println("Key="+key +",csvValue="+ val + ",htmlValue="+formattedKeyVals.get(key)+ ",status=NoMatch ");
				}
			}
			else{
				
				System.out.println("Key="+key +",csvValue="+ val + ",htmlValue="+formattedKeyVals.get(key)+ ",status=KeyValueNotFound");
			}
			System.out.println();
		});
	...
	private static List<String> getRecordsFromCsv(String url) throws Exception{
